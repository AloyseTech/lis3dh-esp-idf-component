...
esp_err_t LIS3DH_ConfigureMotionDetection(uint8_t ODR, uint8_t LPen, uint8_t axisEn, uint8_t enIrqPin1, uint8_t scale,
                                          uint8_t latch, uint8_t D4D, uint8_t threshold, uint8_t duration)
{
    esp_err_t ret = ESP_OK;

    uint8_t ctrl1 = 0b00101111; //((ODR & 0b1111) << 4) | ((LPen & 0b1) << 3) | (axisEn & 0b111);
    uint8_t ctrl2 = 0b00001001; // high pass settings
    uint8_t ctrl3 = 0b01000000; //(enIrqPin1 & 0b1) << 6
    uint8_t ctrl4 = 0b10000000; // 0x80 | ((scale & 0b11) << 4); // scale + BDU
    uint8_t ctrl5 = 0x08;       //((latch & 0b1) << 3) | ((D4D & 0b1) << 2); //latch / 6D
